...
router.get("/artist", isUserLoggedIn, isArtistOrAdmin, (req, res) => {
  res.render("artist/artist-dashboard", {
    userInSession: `Welcome ${req.session.currentUser.username}`,
    userInSessionsId: req.session.currentUser._id,
    buttonA: "Apply for Exhibition",
    linkA: "/artistApplication",
    buttonB: "Favourites",
    linkB: "/artistFavourites",
    buttonC: "Application History",
    linkC: "/artistOrderHistory",
  });
});
...
router.get("/artist/:id", isUserLoggedIn, isArtistOrAdmin, (req, res) => {
  ...
  User.findById(id).then((user) => {
    res.render("artist/artist-details", {
      userInSession: req.session.currentUser,
      user,
      buttonA: "Back To Dashboard",
      linkA: `/artist`,
      buttonB: "Edit Account Details",
      linkB: `/artist/${req.session.currentUser._id}/edit`,
      buttonC: "Application History",
      linkC: "/artistOrderHistory",
    });
  });
...
router.get(
  ...
  async (req, res) => {
    ...
    Promise.all([user, exhibition]).then((values) => {
      ...
      res.render("artist/artist-app-form", {
        user: values[0],
        exhibitionToJoin: exhibitionArray,
        buttonA: "Back To Dashboard",
        linkA: "/artist",
        buttonB: "Favourites",
        linkB: "/artistFavourites",
...
router.get("/artistCart", isUserLoggedIn, isArtistOrAdmin, (req, res) => {
  ...
  ArtistApplication.find({ user }).then((altArtistApp) => {
    res.render("artist/artist-cart", {
      altArtistApp,
      buttonA: "Back To Dashboard",
      linkA: "/artist",
      buttonB: "Favourites",
...
router.post(
  ...
  isUserLoggedIn,
  isArtistOrAdmin,
  fileUploader.single("artworkUrl"),
  async (req, res, next) => {
    ...
    if (req.file) {
      await ArtistApplication.create({
        ...
        .catch(async (err) => {
          ...
          if (
            err._message === "ArtistApplication validation failed" ||
            err instanceof TypeError
          ) {
            let exhibitionToJoin = await Exhibition.find();
            let exhibitionArray = [];
            let user = await User.findById(req.session.currentUser._id);
            Promise.all([user, exhibitionToJoin]).then((values) => {
              values[1].forEach((item) => {
                if (
                  item.exhibitionStatus === "open" &&
                  item.archived === false
                ) {
                  exhibitionArray.push(item);
                }
              ...
              res.render("artist/artist-app-form", {
                user: values[0],
                exhibitionToJoin: exhibitionArray,
                errorMessage: "Error: Please, ensure all fields are complete.",
                buttonA: "Back To Dashboard",
                linkA: "/artist",
                buttonB: "Favourites",
                linkB: "/artistFavourites",
            ...
            res.render("artist/artist-cart", {
              errMsg:
                "You have an application pending. Please, update and submit or delete pending application before starting a new one.",
              buttonA: "Back To Dashboard",
              linkA: "/artist",
              buttonB: "Favourites",
              linkB: "/artistFavourites",
      ...
      Promise.all([user, exhibitionToJoin]).then((values) => {
        ...
        res.render("artist/artist-app-form", {
          user: values[0],
          exhibitionToJoin: exhibitionArray,
          errorMessage: "Error: Please, ensure an image is uploaded.",
          buttonA: "Back To Dashboard",
          linkA: "/artist",
          buttonB: "Favourites",
          linkB: "/artistFavourites",
...
router.get("/artistCart/:id/edit", async (req, res) => {
  ...
  let exhibitionToJoin = await Exhibition.find();
  let exhibitionArray = [];
  Promise.all([artistApptoEdit, exhibitionToJoin]).then((values) => {
    values[1].forEach((item) => {
      if (item.exhibitionStatus === "open" && item.archived === false) {
        exhibitionArray.push(item);
      }
    });
    res.render("artist/artist-cart-edit", {
      artistApptoEdit: values[0],
      exhibitionToJoin: exhibitionArray,
      buttonA: "Back To Cart",
      linkA: "/artistCart",
      buttonB: "Favourites",
      linkB: "/artistFavourites",
      buttonC: "Application History",
...
router.post(
  ...
  (req, res, next) => {
    ...
    ArtistApplication.findByIdAndUpdate(
      id,
      {
        id,
        user,
...
router.get("/artistCart", isUserLoggedIn, isArtistOrAdmin, (req, res) => {
  const user = req.session.currentUser._id;
  ArtistApplication.find({ user }).then((editedItem) => {
    res.render("artist/artist-cart", {
      editedItem,
      buttonA: "Back To Dashboard",
      linkA: "/artist",
...
router.post(
  "/artistCart/:id/delete",
  isUserLoggedIn,
  isArtistOrAdmin,
  (req, res) => {
    const { id } = req.params;
    ArtistApplication.findByIdAndDelete(id).then(() => {
      res.redirect("/artist");
    });
  }
);
...
async function addArtistAppObject(theExhibitionToUpdate, newArtistApp) {
  let updatedExhibition = await Exhibition.updateOne(theExhibitionToUpdate, {
...
router.post(
  ...
  async (req, res) => {
    ...
    for (let i = 0; i < exhibition.length; i++) {
      if (
        exhibition[i].exhibitionWeek ==
        chooseWeek.slice(chooseWeek.length - 29, chooseWeek.length)
      ) {
...
router.get(
  ...
  (req, res) => {
    ...
    Exhibition.find().then((exhibitionsArray) => {
      ...
      res.render("artist/artist-application-history", {
        applicationsArray,
        buttonA: "Back To Dashboard",
        linkA: "/artist",
        buttonB: "Favourites",
...
router.get("/allArtworks", isUserLoggedIn, (req, res) => {
  ...
  let applicationsArray = [];
  Exhibition.find().then((exhibitionsArr) => {
    for (let exhibition of exhibitionsArr) {
      for (let application of exhibition.artistApplication) {
        if (
          exhibition.exhibitionStatus !== "cancelled" &&
          exhibition.archived === false &&
    ...
    if (duplicate) {
      ...
      Exhibition.find().then((exhibitionsArr) => {
        ...
        res.render("artworks", {
          exhibitionsArray,
          applicationsArray,
          errorMsg: "Already added to favourites. Pick new Image",
          buttonA: "Back To Dashboard",
          linkA: "/artist",
          buttonB: "Favourites",
          linkB: "/artistFavourites",
...
async function addFavouriteArtworkToUser(userToUpdate, favouriteArtwork) {
  let updatedUser = await User.updateOne(userToUpdate, {
